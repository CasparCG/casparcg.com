...
To trigger the color output, send a hash tag ## followed by the hexadecimal value of each of the fill channels in the order blue, green and red followed by the value of the key (optional). There are many online colour pickers available to help you select a colour, [ColorPicker.com](https://colorpicker.com) is one of these, Adobe Photoshop and GIMP can also display hexadecimal values. It should be noted that all of these sites allow you to choose RGB hex values but not the key or alpha as they're largely designed for website development.
